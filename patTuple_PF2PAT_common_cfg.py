def createPATProcess(runOnMC, globalTag):
    ...
    ######### Specific filters and counters #########

    process.nEventsTotal = cms.EDProducer("EventCountProducer")
    process.nEventsFiltered = cms.EDProducer("EventCountProducer")

    ## J/psi or D0 flag
    process.jpsid0flag = cms.EDProducer("FlagProducer",
        src         = cms.string("selectedPatJetsPFlow"),
    ...
    ## non iso mu filter
    process.nonisomufilter = cms.EDFilter("NonIsoMufilter",
        src      = cms.string("selectedPatJetsPFlow"),
        minPt    = cms.double(1.),
        )
    ...
    process.out.outputCommands = cms.untracked.vstring('drop *',
        ...
        # 'keep recoPFCandidates_particleFlow_*_*', # need to follow up in the extractor /!\
        'keep PileupSummaryInfos_*_*_*',
        'keep double_kt6PFJets*_rho_*',
        'keep *_patConversions*_*_*',
        'keep *_generalTracks_*_*', # to get track from PFCand from Jet
        'keep recoMuon*_*_*_*', # to get muon from PFCand from Jet
        'keep recoGsfElectron*_*_*_*', # to get el from PFCand from Jet
        'keep *_*nEvent*_*_*', # for the counters 
        'keep *_jpsid0flag_*_*', # for jpsi and d0 flags
        *patEventContentNoCleaning ) 

    process.out.outputCommands += [
      'drop *_selectedPatJetsForMET*_*_*',
      #'drop *_selectedPatJets*_pfCandidates_*', # need to be commented to get PFCand from Jet 
